...
public class StepExecutionService {
    ...
    public UUID processSingleStep(UUID executionId) {
        ...
        boolean isResumingApproval = false;
        java.util.Map<String, Object> dataMap = null;
        com.fasterxml.jackson.databind.ObjectMapper mapper = new com.fasterxml.jackson.databind.ObjectMapper();
        try {
            dataMap = mapper.readValue(execution.getData(), new com.fasterxml.jackson.core.type.TypeReference<java.util.Map<String, Object>>() {});
        } catch (Exception e) {}

        if (currentStep.getStepType() == Step.StepType.APPROVAL) {
            if (dataMap != null && dataMap.containsKey("approved")) {
        ...
        if (isResumingApproval && dataMap != null) {
            dataMap.remove("approved");
            dataMap.remove("approvalReason");
            try {
